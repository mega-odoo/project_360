...
class CreateManualStockPickingWizard(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        po_id = self.env.context.get("default_order_id")

        if not po_id:
            raise ValueError(_('PO not set'))
        purchase_lines = (
        ...
        res["location_dest_id"] = self.env["purchase.order"].browse(po_id)._get_destination_location()
